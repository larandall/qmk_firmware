// generated from users/manna-harbour_larandall/miryoku.org  -*- buffer-read-only: t -*-

#include "larandall.h"

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
#if defined LARANDALL_ALPHAS_COLEMAK
  [BASE] = LAYOUT_larandall(
...
#elif defined LARANDALL_ALPHAS_COLEMAKDH
  [BASE] = LAYOUT_larandall(
...
#elif defined LARANDALL_ALPHAS_DVORAK
  [BASE] = LAYOUT_larandall(
...
#elif defined LARANDALL_ALPHAS_HALMAK
  [BASE] = LAYOUT_larandall(
...
#elif defined LARANDALL_ALPHAS_WORKMAN
  [BASE] = LAYOUT_larandall(
...
#elif defined LARANDALL_ALPHAS_QWERTY
  [BASE] = LAYOUT_larandall(
...
#if defined LARANDALL_NAV_VI
  [NAVR] = LAYOUT_larandall(
  ...
  [NAVR] = LAYOUT_larandall(
   ...
   [NSL]  =  LAYOUT_larandall(
     KC_LBRC,  KC_1,     KC_2,     KC_3,     KC_RBRC,  DF(NAVR), DF(NSL),  DF(BASE), DF(QWER),   RESET,
     KC_COLN,  KC_4,     KC_5,     KC_6,     KC_EQL,   KC_DEL,   KC_LSFT,  KC_LCTL,  KC_LALT,  KC_LGUI,
     KC_GRV,   KC_7,     KC_8,     KC_9,     KC_BSLS,  KC_BSPC,  KC_LEFT,  KC_DOWN,  KC_UP,    KC_RGHT,
     X_NP,     X_NP,     KC_DOT,   KC_0,     KC_MINS,  LT(NSSL,  KC_ENT),   LT(NSL,  KC_SPC),  LT(FUNL,  KC_ESC),  X_NP,  X_NP
   ),
   [NSSL]  =  LAYOUT_larandall(
     KC_LCBR,  KC_EXLM,  KC_AT,    KC_HASH,  KC_RCBR,  DF(NAVR), DF(NSSL), DF(BASE), DF(QWER),   RESET,
     KC_SCLN,  KC_DLR,   KC_PERC,  KC_CIRC,  KC_PLUS,  KC_DEL,   KC_LSFT,  KC_LCTL,  KC_LALT,  KC_LGUI,
     KC_TILD,  KC_AMPR,  KC_ASTR,  KC_LPRN,  KC_PIPE,  KC_BSPC,  KC_LEFT,  KC_DOWN,  KC_UP,    KC_RGHT,
